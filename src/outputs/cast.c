...
struct cast_session
{
  ...
  // register that here so that we on only retry once.
  int retry;
...
cast_cb_startup_launch(struct cast_session *cs, struct cast_msg_payload *payload)
{
  ...
  // Sometimes LAUNCH will not respond (with our requestId). In this case we
  // send a GET_STATUS to see if we are good to go anyway.
  if (!payload && !cs->retry)
    {
	...
	return;
      ...
      DPRINTF(E_LOG, L_CAST, "LAUNCH fallthrough\n");
    }
